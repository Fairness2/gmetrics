...
// G глобальный логер приложения
// G по умолчанию пустой логер
// G по рекомендации документации для большинства приложений можно использовать обогащённый логер, поэтому сейчас используется он, если понадобится, заменить на стандартный логер
var G *zap.SugaredLogger = zap.NewNop().Sugar()
